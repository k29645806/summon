...
class Kobuki(object):
    ...
    def subscribe(self):
        ...
        rospy.Subscriber('/output',String, self.callback_voiceID)  # from recognizer.py
    ...
    def callback_voiceID(self, data):
        self.voiceID = int(data.data)
        time.sleep(1)
        self.voiceID = 0
    def idle(self, final_voice_angle, exist=False):
        ...
        self.state = 'rotate' if (exist and self.voiceID) else 'idle'
    def rotate(self):
        self.fwd_speed = 0.0
    ...
    def forward_state_check(self):
        if self.distance_remain >= self.distance_desire or self.bumper:
...
if __name__ == '__main__':
    while not rospy.is_shutdown():
        ...
        kbk.print_info(ear.possibility)
        if kbk.state == 'idle':
